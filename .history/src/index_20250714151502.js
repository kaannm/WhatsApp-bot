...
app.post('/webhook', verifyWebhook, async (req, res) => {
  ...
  try {
    ...
    if (session.mode === 'ai_photo') {
      return handleAIPhotoMode(req, res, from, session, incomingMessage, now);
    }
...
async function handleAIPhotoMode(req, res, from, session, incomingMessage, now) {
  try {
    // Fotoğraf kontrolü
    if (req.body.image) {
      ...
      switch (session.step) {
        case 0: // Kullanıcı fotoğrafı
          session.data.userImage = imageData;
          session.step++;
          session.attempts = 0;
        ...
        case 1: // Arkadaş fotoğrafı
          session.data.friendImage = imageData;
          session.step++;
          session.attempts = 0;
